...
describe('apiRequest', () => {
  ...
  it('throws ApiError on 401 with body message', async () => {
    globalThis.fetch = (async () => new Response(JSON.stringify({ success: false, message: 'expired' }), { status: 401 })) as unknown as typeof fetch;
  ...
  it('throws ApiError on 400 using error field when message absent', async () => {
    globalThis.fetch = (async () => new Response(JSON.stringify({ success: false, error: 'Bad body' }), { status: 400 })) as unknown as typeof fetch;
  ...
  it('parses JSON body on success', async () => {
    globalThis.fetch = (async () => new Response(JSON.stringify({ success: true, count: 0, cameras: [] }), { status: 200 })) as unknown as typeof fetch;
